  ...
  vim.api.nvim_buf_attach(buffer, false, {
    on_bytes = function(_, buffer, tick, srow, scol, _, oerow, oecol, _, nerow, necol, _)
      vim.schedule(function()
        ...
        local ch_txt = ""
        -- ----------------------------------------------------------------------------
        -- Multi-line or multi-column inserts
        ...
        -- ----------------------------------------------------------------------------
        -- Single char edits and deletions (no range to grab)
          -- false here b/c don't want to produce error with out-of-bounds
          local slice = vim.api.nvim_buf_get_lines( buffer, srow, srow + 1, false)
          if (#slice> 0 and scol < #slice[1]) then
            ch_txt = slice[1]:sub(scol + 1, scol + 1)
          end
        end
