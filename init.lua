...
local two_space_filetypes = { 'xml', 'html', 'xsd', 'lua' }
...
-- Define the desired backup directory relative to the current file's directory
local backup_dir_name = './.nvim_backup'

-- Function to create the directory if it doesn't exist
local function ensure_local_backup_dir(path)
  -- Get the directory of the current file being written
  local file_dir = vim.fn.fnamemodify(path, ':h')
  local full_backup_dir = file_dir .. '/' .. backup_dir_name

  -- Check if the directory exists, and if not, create it
  if vim.fn.isdirectory(full_backup_dir) == 0 then
    -- 'p' creates parent directories as needed
    vim.fn.mkdir(full_backup_dir, 'p')
  end

  -- Return the path Neovim should use for the backup
...
vim.api.nvim_create_autocmd('BufWritePre', {
  callback = function()
    -- Get the full path for the local backup directory
    local local_backup_path = ensure_local_backup_dir(vim.fn.expand '%:p')
    -- Temporarily set backupdir to the newly created local path.
    -- The trailing comma is important: it ensures that if the local
    -- directory isn't writable, it will fall back to the default locations.
    vim.opt_local.backupdir = local_backup_path .. ','
  end,
})
...
require('lazy').setup({
    ...
    config = function()
      ...
      local servers = {
        ...
        clangd = {
          ...
          -- NEW: Add the settings block to control clangd's behavior
          settings = {
            ['clangd.inlayHints'] = {
              -- Explicitly set the common hints to 'false'.
              -- This prevents clangd from overriding Neovim's 'disable' command.
              ParameterNames = false,
              DeducedTypes = false,
              -- Add others like 'Designators' if needed.
            },
          },
  ...
  { -- Autocompletion
    ...
    opts = {
      ...
      -- Somehow this prevents slow ($PATH-related) autocomplete with external ":!" commands
      -- Must be including some other path otherwise??
      cmdline = {
        sources = { 'lsp', 'snippets', 'lazydev', 'path' },
      },
    },
  },
